...
describe("GET /api/payments/callback", () => {
  it("returns a failure HTML page when status=failed", async () => {
    const res = await GET(
      makeRequest("/api/payments/callback", { searchParams: { status: "failed" } }),
    );
  ...
  it("returns a failure HTML page when status=cancelled", async () => {
    const res = await GET(
      makeRequest("/api/payments/callback", { searchParams: { status: "cancelled" } }),
    );
    const html = await res.text();
    expect(html).toContain("Payment Failed");
  });
  ...
  it("returns a processing/success HTML page for any other status", async () => {
    const res = await GET(
      makeRequest("/api/payments/callback", { searchParams: { status: "success" } }),
    );
    const html = await res.text();
  ...
  it("echoes the tx_ref into the processing page", async () => {
    const res = await GET(
      makeRequest("/api/payments/callback", { searchParams: { tx_ref: "rental_abc_123" } }),
    );
    const html = await res.text();
