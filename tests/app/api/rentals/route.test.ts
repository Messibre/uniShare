...
const futureStart = new Date(Date.now() + 1000 * 60 * 60 * 24).toISOString();
const futureEnd = new Date(Date.now() + 1000 * 60 * 60 * 24 * 3).toISOString();
...
describe("POST /api/rentals", () => {
  const validBody = { itemId: "item_1", startDate: futureStart, endDate: futureEnd };

  it("returns 401 when not authenticated", async () => {
    mockedRequireAuth.mockRejectedValue(new Error("Unauthorized"));
    const res = await POST(makeRequest("/api/rentals", { method: "POST", body: validBody }));
  ...
  it("returns 400 when endDate equals startDate", async () => {
    ...
    const res = await POST(
      makeRequest("/api/rentals", {
        method: "POST",
        body: { itemId: "item_1", startDate: futureStart, endDate: futureStart },
      }),
    );
    expect(res.status).toBe(400);
  });
  ...
  it("returns 400 when the item is not AVAILABLE", async () => {
    mockedRequireAuth.mockResolvedValue({ id: "u1", isIdVerified: true } as any);
    mockedItemFindUnique.mockResolvedValue({ ...availableItem, status: "RENTED" } as any);
    const res = await POST(makeRequest("/api/rentals", { method: "POST", body: validBody }));
    expect(res.status).toBe(400);
    const body = await res.json();
  ...
  it("returns 400 when the renter is also the item's owner", async () => {
    mockedRequireAuth.mockResolvedValue({ id: "owner_1", isIdVerified: true } as any);
  ...
  it("computes totalPrice as days * pricePerDay + deposit and creates the rental as PENDING", async () => {
    ...
    mockedRentalCreate.mockResolvedValue({ id: "rental_new", status: "PENDING" } as any);
  ...
  it("treats a missing deposit as 0 when computing totalPrice", async () => {
    mockedRequireAuth.mockResolvedValue({ id: "u1", isIdVerified: true } as any);
    mockedItemFindUnique.mockResolvedValue({ ...availableItem, deposit: null } as any);
    mockedRentalFindFirst.mockResolvedValue(null);
    mockedRentalCreate.mockResolvedValue({ id: "rental_new" } as any);
    mockedStatusLogCreate.mockResolvedValue({} as any);

    await POST(makeRequest("/api/rentals", { method: "POST", body: validBody }));
...
describe("GET /api/rentals", () => {
  ...
  it("returns 500 on unexpected database errors", async () => {
    ...
    const res = await GET(makeRequest("/api/rentals", { headers: { "x-user-id": "u1" } }));
    expect(res.status).toBe(500);
  });
});
