...
import { GET, PATCH, DELETE } from "@/app/api/v1/items/[id]/route";
...
describe("GET /api/items/[id]", () => {
  ...
  it("only includes CONFIRMED/ACTIVE rentals in the availability query", async () => {
    ...
    expect(call.include.rentals.where.status.in).toEqual([
      "CONFIRMED",
      "ACTIVE",
    ]);
  });
...
describe("DELETE /api/items/[id]", () => {
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await DELETE(
      makeRequest("/api/items/item_1", { method: "DELETE" }),
      ctx({ id: "item_1" }),
    );
    expect(res.status).toBe(401);
  });
  ...
  it("returns 404 when the item doesn't exist", async () => {
    ...
    mockedFindUnique.mockResolvedValue(null);
    const res = await DELETE(
      makeRequest("/api/items/missing", { method: "DELETE" }),
      ctx({ id: "missing" }),
    );
    expect(res.status).toBe(404);
  });
